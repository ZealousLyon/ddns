...
for i in "${InterFace[@]}"; do
    ...
    ipv4=$(curl -s4 --max-time 10 --interface "$i" ip.sb -k | grep -E -v '^(2a09|104\.28)' || true)
    ipv6=$(curl -s6 --max-time 10 --interface "$i" ip.sb -k | grep -E -v '^(2a09|104\.28)' || true)

    # 如果第一个接口的 IPv4 地址获取失败，尝试备用接口
    if [[ -z "$ipv4" ]]; then
        ipv4=$(curl -s4 --max-time 10 --interface "$i" https://api.ipify.org -k | grep -E -v '^(2a09|104\.28)' || true)
    fi

    # 如果第一个接口的 IPv6 地址获取失败，尝试备用接口
    if [[ -z "$ipv6" ]]; then
        ipv6=$(curl -s6 --max-time 10 --interface "$i" https://api6.ipify.org -k | grep -E -v '^(2a09|104\.28)' || true)
    fi
